...
namespace modules\tvsinpagar\dlm;

class SynoDLMSearchTVsinpagar
{
    private $qurl = 'http://www.tvsinpagar.com/index.php?page=buscar&q=%s&categoryIDR=%s&idioma=%s&pg=%u';
    private $purl = 'http://www.tvsinpagar.com/';
    private $query = '';
    private $cat = '';
    private $lang = '';
    ...
    private function procesarResultados($plugin, $resultados)
    {
        ...
        foreach ($resultados as $resultado) {
            ...
            #comprobamos si es un enlace a una serie o a una pelicula
            #en el caso de ser una serie recorremos la página con todos sus episodios
            $type = explode('/', $enlacePagina)[3];
            if (strpos($type, 'serie') !== false){
                #es una serie
                if(!in_array($enlacePagina,$this->visitedUrls)){
                    array_push($this->visitedUrls, $enlacePagina);
                    $res = $this->recorrerSerie($plugin, $enlacePagina, $res);
                }
    ...
    private function recorrerSerie($plugin, $enlacePagina, $res){
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_COOKIE, "language=es_ES");
        curl_setopt($curl, CURLOPT_FAILONERROR, 1);
        curl_setopt($curl, CURLOPT_REFERER, $this->purl);
        ...
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_TIMEOUT, 20);
        curl_setopt($curl, CURLOPT_URL, $enlacePagina);
        ...
        do {
            ...
            $res += $this->procesarResultadosSerie($plugin, $resultadosInfo);
            $response = $this->obtenerSiguientePaginaSerie($response, $pagActual, $enlacePagina);
            $pagActual++;
        } while ($response !== null);
    ...
    private function procesarResultadosSerie($plugin, $resultados){
        $res = 0;
        foreach ($resultados as $resultado) {
            $fecha = "{$resultado['anyo']}-{$resultado['mes']}-{$resultado['dia']}";
            $titulo = str_replace('&nbsp;', ' ', strip_tags(iconv('ISO-8859-1', 'UTF-8', $resultado['titulo'])));
    ...
    private function obtenerSiguientePaginaSerie($html, $pagActual, $serieUrl){
        $res = $this->regexp('<ul.*class="pagination".*>.*<a href="(?<enlace_pag_sig>[^>]*)">Next<\/a>.*<\/ul>', $html);
        $ret = null;
        if ($res !== null) {

            $pagUrl = sprintf($serieUrl."/pg/%u", $pagActual + 1);
            $curl = curl_init();
            curl_setopt($curl, CURLOPT_COOKIE, "language=es_ES");
            curl_setopt($curl, CURLOPT_FAILONERROR, 1);
            curl_setopt($curl, CURLOPT_REFERER, $this->purl);
